...
describe('ResetPasswordService', () => {
  ...
  beforeEach(() => {
    ...
    resetPassword = new ResetPasswordService(fakeUsersRepository, fakeUsersTokensRepository, fakeHashProvider);
  });
  ...
  it('should not be able to reset the password with not-existing token', async () => {
    await expect(
      resetPassword.execute({ token: 'not-existing-token', password: '123456' })
    ).rejects.toBeInstanceOf(AppError);
  });

  it('should not be able to reset the password with not-existing user', async () => {
    const { token } = await fakeUsersTokensRepository.generate('not-existing-user');
